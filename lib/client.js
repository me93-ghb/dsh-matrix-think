...
window.__ModuleLoader__.load({
    ...
    `;
        ...
        function SettingsTab() {
            ...
            return React.createElement('div', { className: 'dsh-matrix-think-settings' }, React.createElement('div', { className: 'dsh-matrix-think-settings-copy' }, React.createElement('div', { className: 'dsh-matrix-think-settings-title' }, 'Matrix Think'), React.createElement('div', { className: 'dsh-matrix-think-settings-description' }, 'Turn expanded reasoning into text rain in this browser.')), React.createElement('label', { className: 'dsh-matrix-think-settings-toggle' }, React.createElement('span', null, enabled ? 'Enabled' : 'Disabled'), React.createElement('input', {
                type: 'checkbox',
                role: 'switch',
        ...
        function apply(ctx) {
            ctx.effect(() => ctx.slots.inject('settings.plugins.tab', () => ctx.slots.register({ name: 'settings.plugins.tab', id: 'matrix-think', order: 100, label: 'Matrix Think' }, SettingsTab)), 'matrix-think: settings tab');
            ctx.effect(() => {
                ...
                class MatrixRain {
                    ...
                    syncStreams() {
                        ...
                        for (let index = stableLength; index < this.words.length; index += 1) {
                            ...
                            if (index < this.syncedWords.length) {
                                ...
                                this.pendingWords[column].push(word);
                        }
                        this.syncedWords = this.words.slice();
                    }
                    advanceStream(column) {
                    ...
                    drawWord(word, x, headY, isBlue, still = false, revealed = Infinity) {
                        const context = this.context;
                        if (!context)
                            return;
                    ...
                    ensureBlueRain() {
                        ...
                        for (let column = 0; column < this.streamWords.length; column += 1) {
                            ...
                            if (headY >= 0 && tailY <= this.height)
                                visible.push(column);
                        }
                        if (this.blueColumn !== undefined && visible.includes(this.blueColumn))
                            return;
                        ...
                        if (this.blueColumn !== undefined)
                            this.speeds[this.blueColumn] = BLUE_SPEED;
                    }
                    draw(time) {
                        if (!this.source.isConnected || !this.running)
                            return this.destroyIfDisconnected();
                        this.frame = requestAnimationFrame(next => this.draw(next));
                        if (time - this.lastFrame < FRAME_MS || !this.prepare())
                            return;
                        this.lastFrame = time;
                        this.ensureBlueRain();
                ...
                function scan() {
                    scanFrame = undefined;
                    for (const rain of active) {
                        if (!rain.source.isConnected)
                            rain.destroy();
                    }
                    if (!featureEnabled)
                        return;
                    for (const root of document.querySelectorAll('[data-variant="think"]')) {
                        const row = root.querySelector('[data-disclosure-row][aria-expanded="true"]');
                        const disclosure = row?.parentElement;
                        const source = disclosure?.lastElementChild;
                        if (!(source instanceof HTMLElement) || source === row)
                            continue;
                        const rain = rains.get(source);
                        if (rain)
                            rain.update();
                        else
                            rains.set(source, new MatrixRain(root, source));
                    }
                }
